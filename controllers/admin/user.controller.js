  ...
  try {
    ...
    const addFooter = () => {
      if (!isFirstPage) {
        ...
        doc.fontSize(10)
           .fillColor(colors.textLight)
           .font('Helvetica')
           .text('Generated by GBS', margins.left, footerY, {
             align: 'center',
             width: pageWidth
           });
      }
    ...
    const addDocumentHeader = () => {
      ...
      doc.fontSize(22)
         .fillColor(colors.primary)
         .font('Helvetica-Bold')
         .text('GBS MEMBERS', margins.left + 70, 25);
...
const generateBulkMemberApplicationsPDF = asyncHandler(async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="GBS_Member_Applications.zip"');
...
async function generateApplicationPDF(application, archive) {
  return new Promise((resolve, reject) => {
    ...
    const fileName = `GBS_Application_${application.applicant?.firstName || 'Unknown'}_${
...
function generatePDFContent(pdfDoc, application) {
  ...
  function addHeader() {
    ...
    pdfDoc.fontSize(20)
       .fillColor(colors.primary)
       .font('Helvetica-Bold')
       .text('GBS MEMBERSHIP APPLICATION', margins.left, 30);
  ...
  function addFooter() {
    ...
    pdfDoc.fontSize(9)
       .fillColor(colors.textLight)
       .text('GBS Confidential - For Internal Use Only', margins.left, footerY, {
         align: 'center',
         width: pageWidth
       });
  }
  ...
  currentY = addSection('Application Details', [
    ...
    ['How You Heard About GBS', application.howHeardAbout || 'Not Provided']
  ]);
